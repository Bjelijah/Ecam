...
import com.howell.webcam.InviteResponse;
import com.howell.webcam.NodeDetails;
import com.howell.webcam.NotifyNATResultReq;
import com.howell.webcam.NotifyNATResultRes;
...
import com.howell.webcam.player.PlayerActivity.PlayerHandler;
...
//	public int quickQuit;
    ...
    // private long context; //脥篓鹿媒jni麓芦禄脴ecam_stream_req_context碌脴脰路
        		...
        		if(NATType.equals("UPnP") && !isQuit){
	        		...
	        		try {
						...
						if(count < 25){
						    ...
						    System.out.println("start live start");
						    int ret = Client.this.start(handle, context, REQ_TIMEOUT);
						    isStartFinish = true;
						    if(ret != 0) {
						    	Log.e("false", "postMessage555");
						        System.out.println("PlayerActivity.isQuit :"+isQuit);
...
//                "180.166.7.214", 34780, 0, "100", "100");
    	...
    	if(res == null){
    		Log.e("Client", "res == null");
    	}else{
    		Log.e("Client", res.toString());
    	}
    	try{
